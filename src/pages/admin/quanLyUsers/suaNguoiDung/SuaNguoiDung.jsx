...
export default function SuaNguoiDung() {
  ...
  const { ttNguoiDung, errCapNhat, capNhat } = useSelector(
    ...
    <div className="SuaNguoiDung p-3">
      ...
      <form
        onSubmit={handleSubmit((data) =>
          dispatch(capNhatThongTinNguoiDung({ ...data, maNhom: "GP00" }))
        )}
      >
        {/* Tài khoản */}
        <div>
          ...
          <p className="m-0 text-red-500 h-5">
            {errCapNhat === "Tài khoản đã tồn tại!" ? errCapNhat : ""}
          </p>
        </div>
        {/* Mật khẩu */}
        <div>
        ...
        <div>
          ...
          />
          <p className="m-0 text-red-500 h-5">
            {errCapNhat === "Email đã tồn tại!" ? errCapNhat : ""}
          </p>
        </div>
        {/* Họ tên */}
        <div>
        ...
        <div className="lg:text-right mt-3 flex justify-between">
          <span className="text-green-500 text-xl">
            {capNhat && !errCapNhat ? "Cập nhật tài khoản thành công!" : ""}
          </span>
          {capNhat && !errCapNhat ? (
            ""
          ) : (
